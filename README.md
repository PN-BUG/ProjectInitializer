...
目录、依赖包、插件和设置列表支持按住 Shift 点击首尾复选框，连续勾选或取消勾选。目录树还可以点击目录名称选中行，按 Shift 连选、Ctrl 加选，再用「勾选所选 / 取消所选」统一修改。父目录行「⋯」菜单中的全选/全不选只影响该目录及其子目录；主窗口的概览列表会随窗口高度伸缩。

主窗口的四个步骤以及「从其他 Unity 项目读取」区域都可点击标题折叠，折叠状态会在再次打开窗口时保留。首次打开时跨项目读取、预设概览和日志默认收起；概览标题仍显示本次选中数量。执行初始化时日志区域会自动展开。
