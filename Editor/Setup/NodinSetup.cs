...
namespace ProjectInitializer.Setup
{
    ...
    internal static class NodinSetup
    {
        ...
        /// Nodin 是 ProjectInitializer 的核心依赖，嵌入式包的 package.json 依赖不会被 UPM 自动解析，
        ...
        internal static bool EnsureNodinInManifest()
        {
            ...
            Debug.Log($"[ProjectInitializer] 已自动将 {NodinPackageName} 添加到 manifest.json，Unity 将在下次刷新时解析。");
            return true;
        }
    }
}
